...
function FeaturedProductCard({ product, index, discount }: { product: any, index: number, discount: number }) {
  const [isHovered, setIsHovered] = useState(false)
  const containerVariants = {
  ...
  return (
    <motion.div
      ...
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      className="group relative h-[500px] md:h-[550px] bg-[#1f1f1f] border border-[#2a2a2a] rounded-lg overflow-hidden cursor-pointer shadow-lg hover:shadow-xl transition-shadow duration-300"
    >
...
export function FeaturedCollectionsSection() {
  ...
  useEffect(() => {
    if (totalTestimonialPages <= 1 || isPaused) return
    const interval = setInterval(() => {
      setCurrentTestimonialIndex((prev) => {
        const nextIndex = (prev + 1) % totalTestimonialPages
        if (sliderRef.current) {
          const transformX = -nextIndex * (100 / totalTestimonialPages)
  ...
  }, [totalTestimonialPages, isPaused])
  useEffect(() => {
    ...
    <section className="pt-24 md:pt-32 pb-0 bg-[#0D0D0D] relative overflow-hidden">
      ...
      <div className="relative z-20 pb-24 md:pb-32 bg-[#0D0D0D]">
        ...
        <div className="container mx-auto px-4 relative z-20 pt-12">
          ...
          >
            <AnimatedButton href="/produtos?ofertas=true" />
          </motion.div>
