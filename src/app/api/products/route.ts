...
import { getProducts, getProductImages, getProductVariants, getProductSizes } from '@/lib/database'
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const product of products) {
      ...
      const sizes = productSizes.map((size: any) => size.size).filter(Boolean)
      let badge = null
      let badgeColor = null
      if (product.is_new) {
        badge = 'NOVO'
